...
launchsnake = 'bsub -J unmapped_snake -q long-eth -M 10000 -R "rusage[mem=10000]" "snakemake -s obtain_unmapped_fastq.snake --cluster \\"bsub -q {params.queue} {params.cluster}\\" --jobs 20 --latency-wait 1200 --timestamp"'

x = 0
while x < len(inputfiles):
    readpair = [inputfiles[x],inputfiles[(x+1)]]
    ...
    while (os.path.isfile("filesdone/%s" %First_three_numbers) == False):
        ...
        time.sleep(600)
    #move stuff back to output storage
    #os.system('mv *unmapped* %s' %outdir)
    x=(x + 2)
